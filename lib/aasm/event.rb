module AASM
  ...
  private
    ...
    def current_valid_transitions(obj)
      @transitions.select{ |t| t.from == obj.aasm_current_state }
    end
    ...
    def _fire(obj, test, to_state=nil, *args)
      ...
      if @transitions.map(&:from).any?
        transitions = current_valid_transitions(obj)
      ...
      unless to_state == nil
        if to_state.respond_to?(:to_sym) && 
          !transitions.flat_map(&:to).include?(to_state.to_sym)
